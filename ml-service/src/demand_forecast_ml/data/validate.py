...
def _validate_product_sku(dataframe: pd.DataFrame) -> None:
    product_series = dataframe[PRODUCT_COLUMN]
    ...
    normalized = product_series.astype(str).str.strip()
...
def _parse_dates(dataframe: pd.DataFrame) -> pd.Series:
    ...
    if parsed_dates.isna().any():
        invalid_rows = dataframe.index[parsed_dates.isna()].tolist()
...
def _parse_quantities(dataframe: pd.DataFrame) -> pd.Series:
    ...
    if quantities.isna().any():
        invalid_rows = dataframe.index[quantities.isna()].tolist()
    ...
    non_integer_mask = quantities.mod(1).ne(0)

    if non_integer_mask.any():
        invalid_rows = dataframe.index[non_integer_mask].tolist()

        raise DatasetValidationError(
            f"Column '{TARGET_COLUMN}' must contain integer values. "
    ...
    if negative_mask.any():
        invalid_rows = dataframe.index[negative_mask].tolist()
...
def _validate_unique_key(
    ...
    key_frame = pd.DataFrame(
        {
            PRODUCT_COLUMN: dataframe[PRODUCT_COLUMN].astype(str).str.strip(),
...
def _validate_imputed_column(dataframe: pd.DataFrame) -> None:
    values = dataframe[IMPUTED_COLUMN]
    ...
    if not valid_boolean_mask.all():
        invalid_rows = dataframe.index[~valid_boolean_mask].tolist()
...
def _validate_imputed_quantities(
    ...
    invalid_mask = dataframe[IMPUTED_COLUMN] & quantities.ne(0)
...
def _validate_daily_continuity(
    ...
    for product_sku, group in validation_frame.groupby(
        ...
        invalid_differences = differences.ne(pd.Timedelta(days=1))
...
def _build_summary(
    ...
    return DatasetSummary(
        rows=len(dataframe),
        products=dataframe[PRODUCT_COLUMN]
        .astype(str)
        .str.strip()
        .nunique(),
...
def validate_raw_dataframe(
    ...
    _validate_exact_columns(dataframe, RAW_COLUMNS)
    _validate_product_sku(dataframe)

    parsed_dates = _parse_dates(dataframe)

    _parse_quantities(dataframe)
    _validate_unique_key(dataframe, parsed_dates)

    return _build_summary(dataframe, parsed_dates)
...
def validate_processed_dataframe(
    ...
    _validate_not_empty(dataframe)
    _validate_exact_columns(dataframe, PROCESSED_COLUMNS)
    _validate_product_sku(dataframe)

    parsed_dates = _parse_dates(dataframe)
    quantities = _parse_quantities(dataframe)

    _validate_unique_key(dataframe, parsed_dates)
    _validate_imputed_column(dataframe)
    _validate_imputed_quantities(dataframe, quantities)
    _validate_chronological_order(dataframe, parsed_dates)
    _validate_daily_continuity(dataframe, parsed_dates)
